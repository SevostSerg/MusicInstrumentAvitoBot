...
using AvitoTelegramBot.Sevost.MusicInstrumentAvitoBot.Avito;
...
namespace AvitoMusicInstrumentsBot.InstrumentAnalyzers
{
    public class PriceAnalyzer
    {
        ...
        public PriceAnalyzer(ILogger<Bot> logger)
        {
            _logger = logger;
            _sendedMessagesLogger = logger;
            _titleFilter = new TitleFilter();
            _globalDictionary = new PriceData();
            _telegram = new Telegram(logger);
        }
        ...
        private async Task SendMessage(AdInfo adInfo)
        {
            if (_sent && _titleFilter.IsThisAdTitleGood(adInfo))
            {
                try
                {
                    ...
                    await _telegram.SendMessage(message).ConfigureAwait(false);
                    AutoMessenger.SendAvitoMessage(adInfo.Link);
                    AdsSended++;
